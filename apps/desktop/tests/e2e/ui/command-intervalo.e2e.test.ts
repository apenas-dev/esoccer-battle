/**
 * Intervalo Command E2E Tests
 * Tests the "Intervalo" (pause/resume) command
 */
...
test.describe('Command: Intervalo', () => {
  ...
  test('intervalo.wav audio file should exist', async () => {
    expect(fs.existsSync(audioFiles.intervalo)).toBe(true);
  });

  test('should process intervalo audio via STT endpoint', async () => {
    const audioBuffer = readAudioFile(audioFiles.intervalo);
    
    const formData = new FormData();
    const blob = new Blob([audioBuffer], { type: 'audio/wav' });
    formData.append('audio', blob, 'intervalo.wav');
  ...
  test('should handle pause/resume flow via text', async () => {
